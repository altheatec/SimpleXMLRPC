...
// PRE  : 
// POST : 
class SimpleXMLRPCLoggingClient {
    ...
    function SimpleXMLRPCLoggingClient () {
        ...
        # This value is set in the SimpleXMLRPC client script
        $this->xmlrpcServerURL   = '';
    ...
    // PRE  : 
    // POST : Class members are initialized
    function LogSendData ($data) {
        $prepend = $this->logDelimiter."\nSEND on : ".$this->dateTimeString."\n\n";
        $data    = $prepend.$data."\n";
    ...
    function SetFileName () {
        $currentDate = time ();
        $filename = $this->logdir.'/SimpleXMLRPC_CLIENT_';

        if ($this->logPerAddress) {
            if ($this->xmlrpcServerURL != '') {
                $urlArray = parse_url ($this->xmlrpcServerURL);
                $underscoreName = preg_replace ('/\./', '_', $urlArray['host']);
